...
class TripActivityListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Activity.objects.filter(trip=self.trip).select_related("trip", "created_by")
...
class ActivityDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        return Activity.objects.filter(trip__family__in=user_families).select_related(
            "trip", "created_by"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            membership = FamilyMember.objects.get(
                family=activity.trip.family, user=self.request.user
            )
...
class ActivityCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('Activity "{}" added successfully!').format(activity.name))
...
class ActivityUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy("activities:activity_list", kwargs={"trip_pk": self.object.trip.pk})
...
def rate_activity(request, pk):
    ...
    activity = get_object_or_404(Activity, pk=pk, trip__family__in=user_families)
    ...
    return render(
        request,
        "activities/activity_rating_form.html",
        {
            "form": form,
            "activity": activity,
        },
...
def update_priority(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            user_families = Family.objects.filter(members__user=request.user)
            activity = Activity.objects.get(id=activity_id, trip__family__in=user_families)
            ...
            logger.error("activity_priority_update_failed", error=str(e), user_id=request.user.id)
