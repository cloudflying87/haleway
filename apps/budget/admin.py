...
class BudgetItemInline(admin.TabularInline):
    ...
    fields = ['description', 'estimated_amount', 'actual_amount', 'paid_by', 'payment_date', 'variance']
...
class BudgetCategoryAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'trip', 'color_badge', 'order', 'item_count', 'total_estimated', 'total_actual']
    list_filter = ['trip']
    ...
    total_estimated.short_description = 'Est. Total'

    def total_actual(self, obj):
        """Display total actual amount for this category."""
        return f'${obj.get_total_actual():.2f}'
    total_actual.short_description = 'Actual Total'


@admin.register(BudgetItem)
class BudgetItemAdmin(admin.ModelAdmin):
    """Admin interface for BudgetItem model."""
    list_display = ['description', 'trip', 'category', 'estimated_amount', 'actual_amount',
                    'variance_display', 'paid_by', 'payment_date', 'is_paid']
    list_filter = ['category', 'paid_by', 'payment_date', 'created_at']
    search_fields = ['description', 'notes', 'trip__name']
    readonly_fields = ['id', 'created_at', 'updated_at', 'variance_display', 'variance_percentage_display']
    date_hierarchy = 'payment_date'

    fieldsets = (
        ('Budget Item Information', {
            'fields': ('trip', 'category', 'description', 'notes')
        }),
        ('Amounts', {
            'fields': ('estimated_amount', 'actual_amount', 'variance_display', 'variance_percentage_display')
        }),
        ('Payment Details', {
