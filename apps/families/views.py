...
class FamilyDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pending_invitations'] = family.invitations.filter(
            status='pending'
        ).order_by('-created_at')
...
class FamilyCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            messages.success(
                self.request,
                _('Family "{}" created successfully!').format(self.object.name)
            )
...
def invite_member(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _('Invitation sent to {}!').format(invitation.email)
            )
...
def accept_invitation(request, token):
    ...
    with transaction.atomic():
        ...
        messages.success(
            request,
            _('Welcome to {}!').format(invitation.family.name)
        )
