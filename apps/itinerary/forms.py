...
class ItineraryItemForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ["date", "activity", "title", "time_start", "time_end", "notes", "order"]
        widgets = {
    ...
    def clean(self):
        ...
        if activity and title:
            raise ValidationError(
                _("Please choose either an activity OR a custom event, not both.")
            )
        ...
        if date and self.trip:
            if date < self.trip.start_date or date > self.trip.end_date:
                raise ValidationError(
                    {
                        "date": _(
                            f"Date must be between {self.trip.start_date} and {self.trip.end_date}"
                        )
                    }
                )
        ...
        if time_start and time_end:
            if time_end <= time_start:
                raise ValidationError({"time_end": _("End time must be after start time.")})
