...
class DailyItinerary(models.Model):
    ...
    time_start = models.TimeField(
        _("start time"), null=True, blank=True, help_text=_("Start time (optional)")
    )
    time_end = models.TimeField(
        _("end time"), null=True, blank=True, help_text=_("End time (optional)")
    )
    ...
    notes = models.TextField(_("notes"), blank=True, help_text=_("Additional notes or details"))
    order = models.PositiveIntegerField(
        _("order"), default=0, help_text=_("Display order for items on the same day")
    )
