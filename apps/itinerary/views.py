...
from .forms import AssignActivityForm, ItineraryItemForm, QuickAddEventForm
...
class TripCalendarView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        while current_date <= trip.end_date:
            ...
            items = (
                DailyItinerary.objects.filter(trip=trip, date=current_date)
                .select_related("activity", "created_by")
                .order_by("time_start", "order")
            )
        ...
        scheduled_activity_ids = DailyItinerary.objects.filter(
            trip=trip, activity__isnull=False
        ).values_list("activity_id", flat=True)

        context["unscheduled_activities"] = (
            Activity.objects.filter(trip=trip)
            .exclude(id__in=scheduled_activity_ids)
            .order_by("pre_trip_priority", "name")
        )
...
class DayDetailView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            DailyItinerary.objects.filter(trip=self.trip, date=self.date)
            .select_related("activity", "created_by")
            .order_by("time_start", "order")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        scheduled_activity_ids = DailyItinerary.objects.filter(
            trip=self.trip, activity__isnull=False
        ).values_list("activity_id", flat=True)

        context["unscheduled_activities"] = (
            Activity.objects.filter(trip=self.trip)
            .exclude(id__in=scheduled_activity_ids)
            .order_by("pre_trip_priority", "name")
        )
...
class ItineraryItemCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, _("Added to itinerary for {}").format(item.date.strftime("%B %d"))
        )
...
class ItineraryItemUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "itinerary:day_detail",
            kwargs={"trip_pk": self.object.trip.pk, "date": self.object.date.isoformat()},
        )
...
def quick_add_event(request, trip_pk, date_str=None):
    ...
    return render(
        request,
        "itinerary/quick_add_event.html",
        {
            "form": form,
            "trip": trip,
            "date": event_date,
...
def assign_activity(request, trip_pk, activity_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, _(f"Scheduled {activity.name} for {item.date.strftime('%B %d')}!")
            )
            return redirect("itinerary:day_detail", trip_pk=trip.pk, date=item.date.isoformat())
    else:
    ...
    return render(
        request,
        "itinerary/assign_activity.html",
        {
            "form": form,
            "trip": trip,
            "activity": activity,
