...
class TripPhotoForm(forms.ModelForm):
    ...
    def __init__(self, trip=None, *args, **kwargs):
        ...
        if trip:
            ...
            self.fields['daily_itinerary'].queryset = trip.itinerary_items.all().order_by('date', 'time_start')
            self.fields['daily_itinerary'].empty_label = 'None (general trip photo)'
            self.fields['daily_itinerary'].label_from_instance = lambda obj: f"{obj.date.strftime('%b %d')} - {obj.get_display_title()}"
    ...
    def clean(self):
        ...
        if activity and daily_itinerary:
            if daily_itinerary.activity != activity:
                self.add_error('daily_itinerary',
