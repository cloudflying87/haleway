...
from .forms import DailyJournalForm, TripPhotoForm
...
class TripPhotoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            TripPhoto.objects.filter(trip=self.trip)
            .select_related("uploaded_by", "activity", "daily_itinerary")
...
class TripPhotoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        return TripPhoto.objects.filter(
            trip__family__members__user=self.request.user, uploaded_by=self.request.user
        )
...
class TripJournalListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            DailyJournal.objects.filter(trip=self.trip)
...
class DailyJournalUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        return DailyJournal.objects.filter(
            trip__family__members__user=self.request.user, created_by=self.request.user
        )
