...
class TripNoteListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Note.objects.filter(trip=self.trip).select_related(
            'category', 'created_by'
        )
        ...
        if self.search_form.is_valid():
            ...
            if query:
                ...
                ).filter(search=search_query).order_by('-rank', '-is_pinned', '-updated_at')
...
class NoteDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        ).select_related('trip', 'category', 'created_by')
...
class NoteCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            _('Note "{}" created successfully!').format(self.object.title)
        )
        return response
...
class NoteDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        """Redirect to trip's note list."""
        return reverse_lazy('notes:note_list', kwargs={
...
class NoteCategoryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            _('Category "{}" created successfully!').format(self.object.name)
        )
        return response
